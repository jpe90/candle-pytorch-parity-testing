...
fn build_model_and_tokenizer(args: &Args) -> Result<(XLMRobertaForSequenceClassification, Tokenizer)> {
    ...
    let vb = if weights_filename.ends_with("model.safetensors") {
        ...
        unsafe {
            VarBuilder::from_mmaped_safetensors(&[weights_filename], candle_core::DType::F16, device)?
        }
    } else {
...
fn main() -> Result<()> {
    ...
    if let Some(prompt) = args.prompt {
        ...
        let sentences = vec![
            ...
            "I feel deep regret and sadness about the situation in international politics.".to_string(),
        ];
        println!("Analyzing formality of example texts");
        process_texts(&sentences, &model, &tokenizer, device)?;
    }
